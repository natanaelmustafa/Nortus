...
uses
  LCLIntf,
  SysUtils, Classes, Graphics, Controls, Forms, Buttons, Dialogs, ExtCtrls,
  JvTypes, JvPanel;
...
type
  ...
  TJvColorSpeedButton = class(TSpeedButton) //TJvSpeedButton)
  private
    FDrawColor: TColor;
  ...
  private
    (****************** NOT CONVERTED ************************
    FColorDialogOptions: TJvColorDialogOptions;  // not supported by LCL
    FHotTrack: Boolean;
    FHotTrackFont: TFont;
    FHotTrackFontOptions: TJvTrackFontOptions;
    FHotTrackOptions: TJvHotTrackOptions;        // would require TJvSpeedButton
    procedure SetColorDialogOptions(const Value: TJvColorDialogOptions);
  ...
  protected
    ...
    function CreateUserColors({%H-}ColorList: TStrings): Integer; virtual;
  ...
  public
    ...
    (****************** NOT CONVERTED ********
    property ColorDialogOptions: TJvColorDialogOptions
...
(******************** NOT CONVERTED ********
function JvReaderReadColorDialogOptions(Reader: TReader): TJvColorDialogOptions;
*******************************************)
...
begin
  case Index of
    Tag_NoneColorCaption:
      begin
        Prop := @FShowNoneColor;
        lName := cShowNoneColor;
      end;
    Tag_DefaultColorCaption:
      begin
        Prop := @FShowDefaultColor;
        lName := cShowDefaultColor;
      end;
    Tag_CustomColorCaption:
      begin
        Prop := @FShowCustomColor;
        lName := cShowCustomColor;
      end;
    Tag_ShowAddInHint:
      begin
        Prop := @FShowAddInHint;
        lName := cShowAddInHint;
      end;
    Tag_ShowColorsHint:
      begin
        Prop := @FShowColorsHint;
        lName := cShowColorsHint;
      end;
    Tag_ShowStandardColors:
      begin
        Prop := @FShowStandardColors;
        lName := cShowStandardColors;
      end;
    Tag_ShowSystemColors:
      begin
        Prop := @FShowSystemColors;
        lName := cShowSystemColors;
      end;
    Tag_ShowUserColors:
      begin
        Prop := @FShowUserColors;
        lName := cShowUserColors;
      end;
    Tag_RightClickSelect:
      begin
        Prop := @FRightClickSelect;
        lName := cRightClickSelect;
      end;
    Tag_SelectIfPopup:
      begin
        Prop := @FSelectIfPopup;
        lName := cSelectIfPopup;
      end;
    Tag_HoldCustomColor:
      begin
        Prop := @HoldCustomColor;
        lName := cHoldCustomColor;
      end;
  else
    Exit;
  end;
  ...
  case Index of
    Tag_NoneColorCaption:
      begin
        ...
        lName := cNoneColorColor;
      end;
    Tag_DefaultColorCaption:
      begin
        Prop := @FDefaultColorColor;
        lName := cDefaultColorColor;
      end;
  else
    Exit;
  end;
...
begin
  case Index of
    Tag_NoneColorCaption:
      begin
        Prop := FNoneColorFont;
        lName := cNoneColorFont;
      end;
    Tag_DefaultColorCaption:
      begin
        Prop := FDefaultColorFont;
        lName := cDefaultColorFont;
      end;
    Tag_CustomColorCaption:
      begin
        Prop := FCustomColorFont;
        lName := cCustomColorFont;
      end;
  else
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      X := Width div 2 - 9 + Integer(Down);
      Y := Height div 2 + 4 - Integer(Down);
...
var
  ...
  begin
    while Reader.ReadString <> '' do   // wp: was Reader.ReadStr
      {nothing};
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if Value = '' then Value := ColorsList.ValueFromIndex[I];
...
begin
  ...
  begin
    ...
    with FButtonCustomColor do
      if Visible then
      begin
        TSpeedButton(FButtonCustomColor).Caption := CustomColorCaption;
    ...
    begin
      Control := TControl(FAddInControls[I]);
      ...
      begin
        ...
        begin
          ...
          if Control is TCustomSpeedButton then
...
begin
  ...
  if Sender is TCustomSpeedButton then
    // the PriorCheckedButton have not been assign.
...
(**************************** NOT CONVERTED **************
procedure TJvCustomOfficeColorPanel.SetColorDialogOptions(const Value: TJvColorDialogOptions);
begin
...
begin
  ...
  while Assigned(lParent) do
    if Assigned(TWinControlAccessProtected(lParent).PopupMenu) then
    begin
