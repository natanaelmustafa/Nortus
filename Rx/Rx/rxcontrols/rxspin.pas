...
uses ComCtrls, LCLIntf, LCLType, Controls, ExtCtrls, Classes, GroupedEdit,
...
{$ELSE OLD_SPIN_EDIT}
  ...
  TRxSpinEditControl = class(TGEEdit)
  private
    FEditorEnabled: Boolean;
    procedure WMPaste(var Message: TLMessage); message LM_PASTE;
    procedure WMCut(var Message: TLMessage); message LM_CUT;
  ...
  TCustomRxSpinEdit = class(TCustomAbstractGroupedEdit)
  private
    FOnBottomClick: TNotifyEvent;
    FOnTopClick: TNotifyEvent;
    FArrowKeys: Boolean;
    ...
    function IsMinStored: Boolean;
    function IsValueStored: Boolean;
    procedure SetArrowKeys(AValue: Boolean);
  ...
  TRxSpinEdit = class(TCustomRxSpinEdit)
  private
  public
  published
    property Align;
...
begin
  ...
  begin
    ...
    { buttons frame }
    Pen.Color := clBtnShadow; ///clWindowFrame;
...
procedure TRxSpinEditControl.WMPaste(var Message: TLMessage);
begin
  if not FEditorEnabled or ReadOnly then Exit;
  inherited;
end;

procedure TRxSpinEditControl.WMCut(var Message: TLMessage);
begin
  if not FEditorEnabled or ReadOnly then Exit;
  inherited;
end;

function TRxSpinEditControl.IsValidChar(Key: Char): Boolean;
var
  ValidChars: set of Char;
begin
  ValidChars := ['+', '-', '0'..'9'];
  if TCustomRxSpinEdit(Owner).ValueType = vtFloat then
  begin
    if Pos(DefaultFormatSettings.DecimalSeparator, Text) = 0 then
      ValidChars := ValidChars + [DefaultFormatSettings.DecimalSeparator];
    if Pos('E', AnsiUpperCase(Text)) = 0 then
  ...
  if TCustomRxSpinEdit(Owner).ValueType = vtHex then
  begin
    ValidChars := ValidChars + ['A'..'F', 'a'..'f'];
  end;
  Result := (Key in ValidChars) or (Key < #32);
...
procedure TRxSpinEditControl.KeyPress(var Key: Char);
begin
...
procedure TRxSpinEditControl.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
  if TCustomRxSpinEdit(Owner).ArrowKeys and (Key in [VK_UP, VK_DOWN]) then
  begin
    if Key = VK_UP then TCustomRxSpinEdit(Owner).UpClick(Self)
    else
    if Key = VK_DOWN then TCustomRxSpinEdit(Owner).DownClick(Self);
    Key := 0;
  end;
end;
...
function TCustomRxSpinEdit.GetAsInteger: Longint;
begin
  Result := Trunc(GetValue);
end;

function TCustomRxSpinEdit.GetEditorEnabled: Boolean;
begin
  Result:=TRxSpinEditControl(BaseEditor).FEditorEnabled;
end;

function TCustomRxSpinEdit.GetValue: Extended;
begin
  try
    if ValueType = vtFloat then Result := StrToFloat(Text)
    else if ValueType = vtHex then Result := StrToInt('$' + Text)
...
function TCustomRxSpinEdit.IsIncrementStored: Boolean;
begin
  Result := FIncrement <> 1.0;
end;

function TCustomRxSpinEdit.IsMaxStored: Boolean;
begin
  Result := (MaxValue <> 0.0);
end;

function TCustomRxSpinEdit.IsMinStored: Boolean;
begin
  Result := (MinValue <> 0.0);
end;

function TCustomRxSpinEdit.IsValueStored: Boolean;
begin
  Result := (GetValue <> 0.0);
end;

procedure TCustomRxSpinEdit.SetArrowKeys(AValue: Boolean);
begin
  FArrowKeys := AValue;
end;

procedure TCustomRxSpinEdit.SetAsInteger(AValue: Longint);
begin
  SetValue(AValue);
end;

procedure TCustomRxSpinEdit.SetDecimal(AValue: Byte);
begin
  if FDecimal <> AValue then
  begin
    FDecimal := AValue;
    Value := GetValue;
  end;
...
procedure TCustomRxSpinEdit.SetEditorEnabled(AValue: Boolean);
begin
  TRxSpinEditControl(BaseEditor).FEditorEnabled:=AValue;
end;

procedure TCustomRxSpinEdit.SetValue(AValue: Extended);
begin
  if ValueType = vtFloat then
    Text := FloatToStrF(CheckValue(AValue), ffFixed, 15, FDecimal)
  else
  if ValueType = vtHex then
    Text := IntToHex(Round(CheckValue(AValue)), 1)
  else
    Text := IntToStr(Round(CheckValue(AValue)));
end;

procedure TCustomRxSpinEdit.SetValueType(AValue: TValueType);
begin
...
function TCustomRxSpinEdit.CheckValue(NewValue: Extended): Extended;
begin
  Result := NewValue;
  if (FMaxValue <> FMinValue) then begin
    if NewValue < FMinValue then
...
function TCustomRxSpinEdit.GetEditorClassType: TGEEditClass;
begin
  Result:=TRxSpinEditControl;
end;

function TCustomRxSpinEdit.GetBuddyClassType: TControlClass;
begin
...
procedure TCustomRxSpinEdit.DownClick(Sender: TObject);
var
  OldText: string;
begin
  if ReadOnly then Beep
...
constructor TCustomRxSpinEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Text := '0';
  //ControlStyle := ControlStyle - [csSetCaption];
  FIncrement := 1.0;
  FDecimal := 2;
  TRxSpinEditControl(BaseEditor).FEditorEnabled := True;
  FArrowKeys := True;

  TRxSpinButton(Buddy).FocusControl := BaseEditor;
  TRxSpinButton(Buddy).OnTopClick := @UpClick;
  TRxSpinButton(Buddy).OnBottomClick := @DownClick;

end;

destructor TCustomRxSpinEdit.Destroy;
begin
  inherited Destroy;
end;
{$ENDIF OLD_SPIN_EDIT}
